...
#define MONITOR_PRINT true  // sann om data ska loggas på dataskärmen
#define TRUNC_VAL  1.0 // Någon form av trunkeringsmetod för att slippa enskilda mätvärden som är åt skogen
#define CHECK_FREQUENCY  600  // vänta antal sekunder efter att värmeagg eller luftfuktare har stängts av innan ny beräkning utförs.

#define MAX_TEMP  80.0    // Stäng ner allt permanent om temperaturen överstiger MAX_TEMP
#define MAX_HUMID 100000.0    // - || - överstiger MAX_HUMID
...
#define DT_DT  ((FLYTTAL) 0.010)    // temperaturökning per tidsenhet dTemp / dTid
#define DH_DT  ((FLYTTAL) 0.005)    // luftfuktighetsökning per tidsenhet dHumid / Tid
...
struct {
  ...
  int err;
} state = {false, 0, false, 0, 0, false, 0, 0, OPTIMAL_TEMP, OPTIMAL_HUMID};
...
void updateSettings(FLYTTAL nyHum, FLYTTAL nyTemp)
{
  ...
  //static int numOfErrInARow = 0;
  
    // om någonting är helt åt skogen fel tio gånger i rad så stängs allt ner.
  if (state.err != DHTLIB_OK || nyHum < -100 || nyHum > 200 || nyTemp < -100 || nyTemp > MAX_TEMP)
  {
    if (state.numOfErrInARow++ > 9){
      state.distressAlarm = true; 
    } else  {
      return;
    }
  } else {
    state.numOfErrInARow = 0;
  }
  ...
  if (state.tempValue > OPTIMAL_TEMP+SIGMA_HEAT) {    // Om temperaturen har gått över normalintervall
    ...
    if (state.heatActive == true)
      setHeatActive(false, timeNow);
        ...
        // kolla fukten, trots att temperaturen är för hög.
    if (state.humidValue < OPTIMAL_HUMID-SIGMA_HUMID) {
      setHumidActive(true, timeNow + min((OPTIMAL_HUMID - state.humidValue) / DH_DT, CHECK_FREQUENCY));
    } else if (state.humidValue < OPTIMAL_HUMID-SIGMA_HUMID/2) {
      setHumidActive(true, timeNow + min(2*(OPTIMAL_HUMID - state.humidValue) / DH_DT, CHECK_FREQUENCY));
    }
    ...
    { 
      if (state.humidValue < OPTIMAL_HUMID-SIGMA_HUMID) {
        setHumidActive(true, timeNow + (OPTIMAL_HUMID - state.humidValue) / DH_DT);
      } else if (state.humidValue < OPTIMAL_HUMID-SIGMA_HUMID/2) {
        setHumidActive(true, timeNow + 2*(OPTIMAL_HUMID - state.humidValue) / DH_DT);
      }
...
{
    // kolla om man kan ta bort garble som kommer från displayen
  ...
  if (state.distressAlarm) {
    lcd.print("A");
  } else if (state.numOfErrInARow > 0) {
    lcd.print(state.numOfErrInARow);
  } else {
    lcd.print(" ");
  }
